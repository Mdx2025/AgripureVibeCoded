...
/**
 * Variation E — Cinematic Light with a tinted "in the field" overview video
 * playing behind the hero. Same layout as D; only the hero backdrop changes.
 */
export default function ProductSalesE({ product, related, bundles }: { product: ProductRow; related: ProductRow[]; bundles: Bundle[] }) {
  ...
  return (
    <div className="bg-white pb-20 text-forest">
      {/* HERO — tinted field video behind the content */}
      <section className="relative flex min-h-[calc(100vh-74px)] items-center overflow-hidden bg-paper px-6 sm:px-10">
        ...
        />
        {/* brand tint */}
        <div className="pointer-events-none absolute inset-0" style={{ background: soft, opacity: 0.3 }} />
        {/* readability scrim (strong left, clears to reveal video right) + bottom fade to white */}
        <div
          className="pointer-events-none absolute inset-0"
          style={{
            background:
              "linear-gradient(105deg, rgba(255,255,255,0.93) 0%, rgba(255,255,255,0.62) 38%, rgba(255,255,255,0.12) 72%, rgba(255,255,255,0) 100%), linear-gradient(180deg, rgba(255,255,255,0) 60%, #ffffff 100%)",
          }}
        />

        <div className="relative z-10 mx-auto grid w-full max-w-container items-center gap-10 py-16 lg:grid-cols-[1.1fr_0.9fr]">
          <div>
            <div className="flex items-center gap-3">
              <span className="font-mono text-sm font-semibold" style={{ color: accent }}>No. {product.num}</span>
              <span className="rounded-full border border-black/10 bg-white/80 px-3 py-1 text-[11px] font-bold uppercase tracking-[0.1em] text-fg2 backdrop-blur-sm">{product.type}</span>
            </div>
            ...
            <p className="mt-3 max-w-[540px] text-[16px] leading-[1.6] text-[#3F463E]">{s.sub}</p>
          ...
          <div className="relative flex justify-center">
            {/* white halo separates the carboy from the moving video */}
            <div className="pointer-events-none absolute h-[72%] w-[72%] rounded-full" style={{ background: "radial-gradient(circle, rgba(255,255,255,0.6) 0%, rgba(255,255,255,0) 70%)" }} />
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={bottle} alt={product.name} className="relative h-[clamp(360px,52vh,560px)] w-auto object-contain drop-shadow-[0_40px_60px_rgba(0,40,8,.4)]" />
          </div>
        </div>

        {/* tiny "live footage" cue */}
        <div className="absolute bottom-5 right-5 z-10 hidden items-center gap-2 rounded-full bg-black/35 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-[0.08em] text-white backdrop-blur-sm sm:flex">
